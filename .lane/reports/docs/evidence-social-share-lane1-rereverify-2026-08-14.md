...
present on the current origin/main head `afb5d49` and the live deployment
serves the complete twelve-tag share set with a reachable 1200x630
`og-image.png`. Re-verified fresh on 2026-08-14 — no code change required.

## Why this lane exists

The item re-entered the queue after prior closeouts (2026-08-11 PR #76,
2026-08-12 PR #122, 2026-08-13 PR #168). The prior closeouts verified head
`5209ec7`; this lane re-verifies against the newer current head `afb5d49`
(fresh origin/main checkout) and the live deployment, so the item can close
...
- `git log` confirms the fix commit `eae1d87` ("seo: complete the social
  share image on home (dogfood d87d715be3d0) (#31)") is in history and the
  share set in `public/index.html` is byte-identical to the fix commit; the
  only later head additions are canonical, favicon/icon, and JSON-LD lines
  that do not touch a share tag.
- `public/index.html` `<head>` carries the full twelve-tag Open Graph +
  ...
  content, and `og:image`/`twitter:image` bound to
...
- `public/og-image.png` is a valid non-interlaced 1200x630 PNG, SHA-256
  ...
  byte-identical to the file measured on 2026-08-11/08-12/08-13.
...
### Live deployment (2026-08-14)

- `GET https://tinystudio.io/` → 200; served HTML carries the complete
  twelve-tag share set in `<head>` (all `og:*` and `twitter:*` lines
  present; 5+ lines matching `og:image`/`twitter:image`).
- `GET https://tinystudio.io/og-image.png` → HTTP 200, `content-type:
  image/png`, cache HIT; downloaded bytes SHA-256
  `93109e72af34c2476f2ddd5a7104352e30c13536677e8c6aa7f80be96b3b08df` —
...
receipt, committed on branch `docs/evidence-social-share-lane1-rereverify-2026-08-14`
and opened as a PR against main, closing the item with a reason.
