...
// The legacy page's head must also stop claiming the apex root as its own
// canonical address: while the page was served at the root the canonical and
// og:url pointed at https://tinystudio.io/, which is exactly how the retired
// title consolidated onto the homepage. The legacy page now declares its own
// canonical .html twin (https://tinystudio.io/agent-desk.html, the address
// the worker serves and the form every other owned page canonicals to), so
// neither a search engine nor a scraper can fold the retired surface's
// identity into the homepage again.
