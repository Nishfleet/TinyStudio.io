...
`origin/main` head (`92d55c3`, "fix(check): guard the apple touch icon on
every served page, and re-verify finding 98a7bf8e08fc (2026-08-20)
(#256)"). It is a source-side plus served-bytes re-verify; no live
AI-search capture or browser-side rendering check is in scope.
...
release cycles. The 2026-08-20 round-1 harvest closeout (PR #246,
`743cdb7`) re-verified the framing against head `d0daea9`; the
intervening commits since that re-verify are all toolchain or feature
work that does not touch any of the eight framing clauses the
product-contract test guards:

- `0540cf9` — Merge PR #247 (`docs/render-blocking-reverify-2026-08-20`).
- `75cfef2` — `fix(public): keep /pricing lead-form bare inputs at a 44px
  tap target (#251)` (tap-target fix on `pricing.html`, not on the
  active product-contract surface).
- `92d55c3` — `fix(check): guard the apple touch icon on every served
  page, and re-verify finding 98a7bf8e08fc (2026-08-20) (#256)`
  (apple-touch-icon fix, unrelated to the product-contract framing).

No commit between `743cdb7` and this head changes any of the eight
framing clauses the product-contract test guards, and no commit moves
the `> **Status: CURRENT.**` banner off `specs/004-website-appraisal/plan.md`,
the `> **Status: HISTORICAL — retired.**` banner off specs 001/002, or
the `> **Status: SUPERSEDED — historical campaign plan.**` banner off
spec 003.
...
5. **Full test suite** — `node --test scripts/test-heading-hierarchy.mjs scripts/test-sitemap.mjs scripts/test-agent-worker.mjs scripts/test-agent-ui.mjs scripts/test-product-contract.mjs scripts/test-first-viewport-audience.mjs`
   → 124 tests, 124 pass, 0 fail (heading-hierarchy 6 + sitemap 7 +
   agent-worker 83 + agent-ui 16 + product-contract 8 + first-viewport-
   audience 4 = 124). The agent-worker count grew from 76 in the 2026-08-14
   receipt to 83 in this head, reflecting the 2026-08-15 six-a-month
   intake-cap rebase of PR #116 (`dda25f2`, merged `2026-08-20`); no
   contract framing changed.
6. **Site-wide checker** — `node scripts/check-site.mjs` → exit 0,
   "TinyStudio.io checks passed." The narrow-viewport probe
   (`test-narrow-viewport-pages.mjs`) and the render-blocking probe
   (`check-render-blocking.mjs`) are not part of this re-verify.
...
`origin/main` (head `92d55c3`). The proof files added by this lane are
documentation only:
