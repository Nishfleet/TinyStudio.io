...
1. Copy `public/` to `fixed-site/`; serve on 127.0.0.1 (e.g. `python3 -m http.server 8131 --directory fixed-site`).
2. For the unfixed variant, copy again to `unfixed-site/` and strip the mobile block from its `audit.css`:
