...
This receipt originally landed on branch `lane1/ai-search-rerun-entity-offer-20260812`
as PR #128, which went CONFLICTING against main and was stranded. On
2026-08-15 the re-run was re-landed on a fresh branch from current
`origin/main` (05efed1) with two reconciliations:

1. `controlled-questions.json` now carries main's q5 ground truth ("the
   free leak audit of high-ticket service homepages, and the human-reviewed
   desk that closes what the audit finds", merged via PR #43 / ed62202)
   rather than the retired Agent Desk wording the original branch carried.
   The yardstick change is a repository-side correction already on main; the
   2026-08-12 captured runs are untouched and are recorded against the
   current truth.
2. The fixture checks in `scripts/test-agent-ui.mjs` were updated for the
   `absent` state the 2026-08-12 Bing runs introduced (the 2026-08-09
   fixture carried no `absent` runs).
