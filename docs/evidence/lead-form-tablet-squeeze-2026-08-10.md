...
## Re-verified 2026-08-11 (reprise PR)

The fix branch above was never merged, so the same receipt was re-run independently against fresh `origin/main` HEAD (`dc95ebf`, 2026-08-11) before re-opening this PR. Unfixed, using the same static-server method (Playwright 1.62.1, Chromium headless, Karla confirmed loaded):

| Viewport | layout (unfixed) | i1/i2 (unfixed) |
|---|---|---|
| 761 | row, gap 70px | **88 / 87** |
| 768 | row, gap 70px | **103 / 102** |
| 800 | row, gap 70px | 119 / 118 |
| 834 | row, gap 70px | 136 / 135 |
| 900 | row, gap 70px | 139 / 138 |
| 901 | row, gap 70px | 139 / 138 |
| 1024 | row, gap 70px | 139 / 138 |
| 1280 | row, gap 70px | 139 / 138 |

Placeholder clipped (`phFits: false`) at every width; `scrollOK: true` everywhere. The 761px row (88/87px) is slightly narrower than the original run's 99/98px — font-metric variance between runs — same bug, same conclusion. Fixed-state numbers in the table above were re-confirmed at 761/768/834/900/901/1024px with this branch's `index.css` (column 623/630/696/762/762px stacked; row 209/208px ≥901px; mobile 390px 332/332px identical).
