# Product-contract candidate round 1: harvest re-verification (2026-08-20)
...
Agent Desk as current") against the current `origin/main` (`d0daea9`) and
live, so the tracker item stays closed against drift and cannot re-open.

This receipt extends `product-contract-round1-harvest-2026-08-17.md` (the
2026-08-17 re-verification) onto the current main (`d0daea9`) and live
(2026-08-20).
...
Re-checked on 2026-08-20:

- All seven candidate worktrees still exist on disk; every branch is pushed
  to `origin/candidate/*` and in sync; the only uncommitted item in each
  is a deleted Python bytecode cache
...
- The abandoned lane-1 worktree still holds the two uncommitted
  working-tree refinements on top of `96a10b3` (the stash commit `939e6c4`)
  noted in the 2026-08-17 receipt. No new work has been added there since
  the 2026-08-10 closure.
...
closed by PR #99. Subsequent re-verifications on 2026-08-11, 2026-08-12,
2026-08-13, 2026-08-14, and 2026-08-17 have confirmed the work is fully
represented in main.

## Verification performed on the current head (`d0daea9`, 2026-08-20)

1. **Current plan marker** — `git show origin/main:specs/004-website-appraisal/plan.md`
   opens with `> **Status: CURRENT.** The Website Appraisal is TinyStudio's
   ...
   plan.`
2. **Historical banners** — `specs/001-public-buyer-page/plan.md` and
   `specs/002-minimal-input-agent-desk/plan.md` each open with
   `> **Status: HISTORICAL — retired.**` and point at the current plan.
...
6. **Plan Boundaries** — `specs/004-website-appraisal/plan.md` discloses
   that `/api/signups` stores the submitted email and the normalized
   submitted website URL in D1 alongside lightweight request metadata —
   byte-identical to the abandoned draft.
7. **Product-contract guard** — `scripts/test-product-contract.mjs` carries
   the refactored superset of the abandoned draft's semantics (bounded
   leading-banner status declaration, per-claim Agent Desk evaluation
   over `matchAll`, negation-precedes-term guarantee check, and the
   `WEBSITE_STORED` / `WEBSITE_NOT_STORED` disclosure guards).
8. **Repository-descriptor drift between PR #58 (`11864a7`) and current
   main (`d0daea9`)** — only toolchain/feature commits (wrangler
   `^4.120.1` bump, the 2026-08-17 favicon SVG landing, the GoodFirms /
   Clutch / render-blocking / apple-touch-icon / AI-search /
   pipeline-bridge re-verifications, the retired-Agent-Desk canonical
   fix, the appraisal `autocomplete="email"` patch, the 2026-08-15
   controlled entity-and-offer re-run, the 2026-08-18 six-a-month
   intake-cap rebase of PR #116); zero contract drift. README, MEMORY,
   and package.json still name The Website Appraisal as the current
   offer.
...
- `docs/evidence/product-contract-round1-harvest-2026-08-20.md` — new
  evidence receipt recording the 2026-08-20 re-verification on the current
