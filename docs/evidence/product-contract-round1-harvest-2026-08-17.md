...
(the seven-candidate closeout), `product-contract-round1-harvest-2026-08-13.md`
(the first re-verification), and
`product-contract-round1-harvest-2026-08-14.md` (the second re-verification)
onto the current main (`5ca6241`) and live (2026-08-17).
...
## Verification on the current head (`5ca6241`, 2026-08-17)
...
- `scripts/test-product-contract.mjs`: main carries a refactored superset of
  ...
  - per-claim `AGENT_DESK_ACTIVE_PATTERNS` evaluation with the same
    ...
    first match (lines 149, 176, 189, 230);
...
- Repository-descriptor drift between PR #58 (`11864a7`) and current main:
  only toolchain/feature commits (wrangler `^4.120.1` bump, the
  `2026-08-17` favicon SVG landing, the GoodFirms / Clutch /
...
- `specs/003-wellness-clinic-launch/plan.md` - `Status: SUPERSEDED - historical campaign plan.`;
- `specs/004-website-appraisal/plan.md` - `Status: CURRENT.`.

Test suite on a fresh `origin/main` checkout (`5ca6241`):

- `node scripts/check-site.mjs` - "TinyStudio.io checks passed."
- `node --test scripts/test-product-contract.mjs` - 8/8 (including the
  "known bad shape" fixtures proving the guard rejects the regressions it
  guards, and the `WEBSITE_STORED` boundary-disclosure assertion).
...
`origin/main` (`5ca6241`), the deterministic guard passes 8/8, the full
...
work harvested through PR #58 and re-verified against `5ca6241` and live
on 2026-08-17; nothing further to change."
