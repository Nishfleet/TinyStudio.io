...
2. Uses remote bindings (production D1, production AI) — this is the only supported dev mode; there is no local-only fixture mode.
...
- `wrangler dev --local` or any local-only mode — the app has no local fixture support and the migrations are only applied remotely.
...
- `GET /agent-desk` → 200, static page explaining the self-serve desk is retired and pointing to the public site.

### Test-only surfaces — never drive these

- The AI binding is only exercised by the retired `/api/agent-audit` path. Do not drive it for the current product.
- The `app.tinystudio.io` and `api.tinystudio.io` hosts return `410 Gone` with retired messaging; verifying them is out of scope for a 127.0.0.1:8788 run.
