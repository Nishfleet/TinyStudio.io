...
4. `remediation` is page-specific (`remediation.page`) only when this run's
   ...
   verify the same-domain link. Otherwise remediation says plainly that no
   page-specific fix is claimed. Even with a same-domain source, remediation
   claims only what that cited page actually supports: it never asserts that a
   page produced a description the page does not carry.
